...
def generate_notes():
    try:
        ...
        if not user_answers or not questions:
            return jsonify({
                'status': 'error',
                'error': 'No quiz data available. Please take the quiz first.'
            }), 400

        # Prepare questions text with explanations
        questions_text = ""
        for q in user_answers:
            questions_text += f"Q: {q['question']}\n"
            questions_text += f"Correct Answer: {q['correct_answer']}\n"
            questions_text += f"Explanation: {q['explanation']}\n\n"

        # Create a more focused prompt for study notes
        prompt = f"""
        Create comprehensive study notes based on the following quiz content. The notes should be educational and helpful for learning:
